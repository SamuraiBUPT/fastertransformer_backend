...
if __name__ == '__main__':
    ...
    trace = Trace(args, time_period=100, text_directory='./dataset/cc_new/txt')
    ...
    def warm_up(_inputs):
        ...
        url = "http://localhost:8003/arrival"
        response = requests.post(url, json=data)
        # result = client.infer(model_name, _inputs)
        whole_time = time.time() - start_time
        return whole_time, response
    ...
    def sync_request_pull_grpc(_model_name, 
        ...
        req_delay_pull_time = 3    # change here to a suitable value
        preferred_batch_size = 2    # change here for preferred batch size
        ...
        while pushed_request < max_request:
            ...
            req_sequence_idx += 1
            time.sleep(req_delay_pull_time)


        while(True):
            if len(user_data) == max_request:
                print(f"[INFO] {max_request} requests received")
                break
